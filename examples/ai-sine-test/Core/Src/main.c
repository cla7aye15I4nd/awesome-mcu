...
int main(void)
{
  ...
  oled_init();
  char buffer[32];
  oprintf(0, "  tf-lite model");
  
  AI_ALIGNED(32) ai_u8 activations[AI_SINEMODEL_DATA_ACTIVATIONS_SIZE];
  AI_ALIGNED(32) ai_i8 in_data[AI_SINEMODEL_IN_1_SIZE_BYTES];
  ...
  ai_buffer ai_input[AI_SINEMODEL_IN_NUM] = AI_SINEMODEL_IN;
  ai_buffer ai_output[AI_SINEMODEL_OUT_NUM] = AI_SINEMODEL_OUT;

  ai_network_params ai_params = AI_NETWORK_PARAMS_INIT(
    AI_SINEMODEL_DATA_WEIGHTS(ai_sinemodel_data_weights_get()),
    AI_SINEMODEL_DATA_ACTIVATIONS(activations)
  );
  ...
  ai_err = ai_sinemodel_create(&sinemodel, AI_SINEMODEL_DATA_CONFIG);
  if (ai_err.type != AI_ERROR_NONE) {
    oprintf(1, "Create Error");
    while(1);
  }
