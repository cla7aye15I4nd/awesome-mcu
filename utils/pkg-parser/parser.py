...
parse_type = 'stm32f4'
c_header_path = '../../packages/STM32F4/Drivers/CMSIS/Device/ST/STM32F4xx/Include'

def readfile(filename):
    with open(os.path.join(c_header_path, filename)) as f:
        return f.read()
    ...
    with open(os.path.join('examples', f'{parse_type}.py'), 'w') as f:
        ...
        for perip_name, perip_set in perip.items():
            ...
            for item in perip_set:
                ...
                elif 'stm32f411xe.h' in filenames or len(perip_set) == 1:
...
def generate_config(c_header_file, perip):
    ...
    config['FLASH'] = {
        'type' : 'memory',
        'size': hex(base_address['FLASH_END'] - base_address['FLASH_BASE'] + 1), # write in comment
